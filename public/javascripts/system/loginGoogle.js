...
function($scope) {
    ...
    $scope.loginGoogle = function() {
        ...
        //     alert('TĂ i khoáº£n email nĂ y Ä‘Ă£ Ä‘Æ°á»£c dĂ¹ng Ä‘á»ƒ xĂ¡c thá»±c Ä‘Äƒng nháº­p báº±ng Facebook');
        //     window.location.href = '/';
        // });
        ...
        firebase.auth().signInWithPopup(new firebase.auth.GoogleAuthProvider())
            .then(function(result) {
                // console.log('first login google');
                // This gives you a Google Access Token. You can use it to access the Google API.
                var token = result.credential.accessToken;
                // The signed-in user info.
                var user = result.user;
                firebase.database().ref('listUsers/' + user.uid).once('value')
                    .then(function(snapshot) {
                        ...
                        if (snapshot.val() === null) {
                            ...
                            // console.log('just have user');
                            window.location.href = '/';
                        }
                    });
            })
            .catch(function(error) {
                // console.log('logined with other');
                // An error happened.
                if (error.code === 'auth/account-exists-with-different-credential') {
                    ...
                    auth.fetchProvidersForEmail(email).then(function(providers) {
                        ...
                        */
                        ...
                        // console.log('maybe logined with facebook');
                        ...
                        var alert = confirm("Báº¡n Ä‘Ă£ Ä‘Äƒng nháº­p tĂ i khoáº£n nĂ y báº±ng Facebook. Báº¡n cĂ³ muá»‘n liĂªn káº¿t nĂ³ vá»›i tĂ i khoáº£n Google?");
                        if (alert == true) {
                            auth.signInWithRedirect(provider).then(function(result) {
                                ...
                                result.user.link(pendingCred).then(function() {
                                    // console.log('link account gg & fb');
                                    // Google account successfully linked to the existing Firebase user.
                                    window.location.href = '/';
                                });
                            });
    ...
    function addUserToDatabase(user) {
        // console.log('add User with Google', user);
